...
//                    Mail::to("a.allahverdi@icoa.it")->send(new Welcome($user->nome,$user->email,$user->codice,$password));
                ...
                $row=DB::connection('mysql')->table('ec_customers')->where('codice', $user->codice)->update([
                    'codice' => $user->codice,
                    'name' => $user->nome,
                    'codice_fiscale' => $user->codice_fiscale,
                    'piva' => $user->piva,
        ...
        DB::connection('mysql')->table('ec_customers')->whereIn('codice', $usersToRemove)->update(['status' => 'locked']);
            ...
            \Illuminate\Support\Facades\DB::transaction(function () use ($products,$productsWithoutVariants, $variants, $brands, $items) {
                ...
                foreach ($productsWithoutVariants as $productsWithoutVariant) {
                    ...
                    if (in_array($product_name, $items)) {
                        ...
                        $productItem = $this->_generateProduct($id,$product_name,$productsWithoutVariant,$price,$brands,$taxId,$sku);
                ...
                foreach ($variants as $product_name=>$products) {
                    ...
                    if (in_array(str_replace('&', 'and', trim($product_name)), $items)) {
                        ...
                        $productItem = $this->_generateProduct($id,$product_name,$product,$price,$brands,$taxId,$sku);
                        $this->_generateTranslationProduct($product_name,$productItem);
                        $this->_generateSlugProduct($product_name,$productItem);
